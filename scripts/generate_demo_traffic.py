...
def run_round(base_url: str, round_index: int, skip_llm: bool) -> None:
    ...
    for method, path, payload in calls:
        status, preview = request_json(base_url, method, path, payload)
...
def main() -> None:
    ...
    parser.add_argument("--rounds", type=int, default=3, help="Nombre de tours de requêtes à envoyer.")
    parser.add_argument("--delay", type=float, default=1.0, help="Pause en secondes entre deux tours.")
    parser.add_argument("--skip-llm", action="store_true", help="N'appelle que /health et /predict.")
    args = parser.parse_args()

    for index in range(args.rounds):
        print(f"\n--- Round {index + 1}/{args.rounds} ---")
        run_round(args.base_url, index, args.skip_llm)
